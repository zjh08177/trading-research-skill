...
def _pack(primary=None, tiingo=None, finnhub=None, asof="2026-07-17"):
    def f(v):
        return None if v is None else {"v": v, "unit": "USD", "asof": asof, "src": "test"}
    pack = {}
    if primary is not None:
        pack["P1.last"] = f(primary)
...
def test_ok_when_primary_and_tiingo_agree_no_finnhub_needed():
    facts = mod.build(_pack(primary=100.0, tiingo=100.2))
    assert facts["P1.crosscheck_status"]["v"] == "ok"


def test_unavailable_when_baseline_sources_missing():
    facts = mod.build(_pack(primary=100.0))  # no tiingo
    assert facts["P1.crosscheck_status"]["v"] == "unavailable"


def test_fail_unresolved_when_disagree_and_no_finnhub():
    facts = mod.build(_pack(primary=100.0, tiingo=105.0))
    assert facts["P1.crosscheck_status"]["v"] == "fail_unresolved"


def test_resolved_2of3_primary_finnhub_outvote_tiingo():
    facts = mod.build(_pack(primary=100.0, tiingo=105.0, finnhub=100.1))
    assert facts["P1.crosscheck_status"]["v"] == "resolved_2of3"
    assert facts["P1.crosscheck_resolved_price"]["v"] == 100.0


def test_resolved_2of3_tiingo_finnhub_outvote_primary():
    facts = mod.build(_pack(primary=100.0, tiingo=105.0, finnhub=105.1))
    assert facts["P1.crosscheck_status"]["v"] == "resolved_2of3"
    assert facts["P1.crosscheck_resolved_price"]["v"] == 105.0


def test_fail_3way_when_all_pairwise_disagree():
    facts = mod.build(_pack(primary=100.0, tiingo=105.0, finnhub=110.0))
...
def test_fail_3way_when_finnhub_agrees_with_both_but_they_disagree_with_each_other():
    # a wide tolerance edge case: finnhub sits close to both, but primary and
    # tiingo are still outside tolerance of each other -- no clean majority
    facts = mod.build(_pack(primary=100.0, tiingo=100.9, finnhub=100.45), )
    # with default 0.5% tolerance this is primary-tiingo disagree (0.9%),
    # primary-finnhub agree (0.45%), tiingo-finnhub agree (0.45%) -> both agree
    assert facts["P1.crosscheck_status"]["v"] == "fail_3way"
...
def test_real_live_aapl_snapshot_agrees():
    ...
    facts = mod.build(_pack(primary=333.9988, tiingo=333.85, finnhub=333.74))
    assert facts["P1.crosscheck_status"]["v"] == "ok"


def test_main_cli_smoke(tmp_path, capsys):
    p = tmp_path / "10-datapack.json"
    p.write_text(json.dumps(_pack(primary=100.0, tiingo=105.0, finnhub=100.1)))
    code = mod.main([str(p)])
