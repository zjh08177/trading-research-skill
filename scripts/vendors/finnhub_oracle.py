...
source, fetched only when schwab (primary) and tiingo (existing oracle)
already disagree beyond tolerance — see scripts/price_crosscheck.py, which
resolves the 2-of-3 vote. Current-day only: Finnhub /quote has no as-of
parameter, it always returns the live/last-session price, so a past/future
--asof is refused rather than silently mislabeled (never look-ahead).
